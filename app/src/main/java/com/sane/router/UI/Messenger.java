...
public class Messenger implements Runnable {
    ...
    private List<Record> validNetworkList;		  // the route list containing current valid remote networks.
    ...
    public void finishCreatingMessenger(){
        parentActivity = ParentActivity.getParentActivity();//TODO: Is this correct?
    ...
    private View.OnClickListener sendMessage = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            layer3Daemon.sendMessage(messageEditText.getText().toString(), destinationLL3P);
    ...
    public void receiveMessage(Integer ll3pAddress, String payload){
        if (messengerWindow.isShowing()){
            int network = (Integer) (ll3pAddress/256); // = address/256 with no remainder.

            // note that in original "256" comes from network constants class.
            if (network >= 0 && network <16){
                String name = (allNamesAndNetworks.get(network-1).name);
    ...
    private void resetSpinner(){
        ...
        for (TableRecord entry : validNetworkList){
            if (((Integer)((RoutingRecord)entry).getNetworkNumber()).compareTo(-1) > 0 &&
                ((Integer)((RoutingRecord) entry).getNetworkNumber()).compareTo(16) < 0)
